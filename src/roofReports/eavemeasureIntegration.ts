...
export const EAVEMEASURE_AERIAL_SERVICES_URL =
  (typeof process !== "undefined" &&
    (process as { env?: Record<string, string | undefined> }).env?.EXPO_PUBLIC_EAVEMEASURE_URL) ||
...
export function roofMeasurementsHaveContent(m?: RoofMeasurements): boolean {
  if (!m) return false;
  return !!(
    (typeof m.roofAreaSqFt === "number" && Number.isFinite(m.roofAreaSqFt) && m.roofAreaSqFt > 0) ||
    (typeof m.roofPerimeterFt === "number" && Number.isFinite(m.roofPerimeterFt) && m.roofPerimeterFt > 0) ||
...
export async function openEaveMeasureAerialServices(): Promise<void> {
  ...
  try {
    ...
    console.error(e);
    Alert.alert("Could not open EaveMeasure", "Copy the URL from the in-app caption and open it in your browser.");
  }
}
