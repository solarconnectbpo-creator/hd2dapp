...
function pickCombinedName(row: Record<string, unknown>, firstNameKey: string | null, lastNameKey: string | null): string {
  const first = firstNameKey && row[firstNameKey] != null ? String(row[firstNameKey]).trim() : "";
  const last = lastNameKey && row[lastNameKey] != null ? String(row[lastNameKey]).trim() : "";
...
export function parsePropertyLeadsCsvText(csvText: string): ParsedCsvResult {
  ...
  const addressKey = pickFirstHeaderKey(first, ["address", "addr", "property", "fulladdress", "streetaddress"]);
  ...
  const firstNameKey = pickFirstHeaderKey(first, ["firstname", "first_name", "first name", "fname", "ownerfirst", "owner_first"]);
  const lastNameKey = pickFirstHeaderKey(first, ["lastname", "last_name", "last name", "lname", "ownerlast", "owner_last"]);
  ...
  const roofSqFtKey = pickFirstHeaderKey(first, ["roof_sqft", "roofarea_sqft", "roofarea", "roof_area", "roofsize", "roof_sqft_size", "roof sq ft"]);
  const roofTypeKey = pickFirstHeaderKey(first, ["roof_type", "roofmaterial", "roof_material", "material"]);
  ...
  rows.forEach((row, idx) => {
    ...
    const homeownerNameCombined = pickCombinedName(row, firstNameKey, lastNameKey);
    const homeownerName = homeownerNameDirect || homeownerNameCombined;
    const email = emailKey && row[emailKey] != null ? String(row[emailKey]).trim() : "";
    const phone = phoneKey && row[phoneKey] != null ? String(row[phoneKey]).trim() : "";
    ...
    const normalizedRoofSqFt =
      typeof roofSqFt === "number" && Number.isFinite(roofSqFt) ? roofSqFt : undefined;

    const roofType =
      roofTypeKey && row[roofTypeKey] != null ? String(row[roofTypeKey]).trim() : "";
