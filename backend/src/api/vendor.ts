...
export async function deliverLeads(req: Request, env: Env) {
  try {
    ...
    if (!vendorId) {
      return new Response(JSON.stringify({ error: "Missing x-company-id header" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
    ...
    if (!product_id || !Array.isArray(leads) || leads.length === 0) {
      return new Response(JSON.stringify({ error: "Missing product_id or leads array" }), {
        status: 400,
        headers: { "Content-Type": "application/json" }
      });
    }
    ...
    ).bind(product_id, vendorId).first();
