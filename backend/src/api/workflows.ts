 ...
 */
export async function getWorkflow(req: Request, env: Env, user: User, workflowId: string) {
  try {
 ...
 */
export async function toggleWorkflow(req: Request, env: Env, user: User, workflowId: string) {
  try {
    const body = await req.json();
    const { active } = body;

    await env.DB.prepare(
      "UPDATE workflows SET active = ? WHERE id = ?"
    ).bind(active ? 1 : 0, workflowId).run();
    ...
    return new Response(JSON.stringify({ error: "Failed to toggle workflow" }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
  }
 ...
 * Delete workflow
 */
export async function deleteWorkflow(req: Request, env: Env, user: User, workflowId: string) {
  try {
    // Delete steps
    await env.DB.prepare(
