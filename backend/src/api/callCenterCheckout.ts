...
export type CallCenterCheckoutEnv = AuthEnv & {
  STRIPE_SECRET_KEY?: string;
  /** Monthly call-center subscription Price id (see wrangler.toml [vars]). */
  CALLCENTER_STRIPE_PRICE_ID?: string;
  APP_PUBLIC_ORIGIN?: string;
};

/**
 * POST /api/billing/callcenter-checkout-session
 *
 * Starts a Stripe Checkout subscription for the call-center add-on. Tagged with
 * `hd2d_checkout_kind=callcenter` so the shared webhook does not treat it as platform
 * membership billing (see stripeWebhook.ts).
 */
export async function handleCallCenterCheckoutSession(
  request: Request,
  env: CallCenterCheckoutEnv,
  ...
  const priceId = (env.CALLCENTER_STRIPE_PRICE_ID || "").trim();
  if (!priceId) {
    return new Response(
      JSON.stringify({
        success: false,
        error: "Call center pricing is not configured. Set CALLCENTER_STRIPE_PRICE_ID on the Worker.",
      }),
  ...
  params.set("success_url", `${origin}/call-center?checkout=success&session_id={CHECKOUT_SESSION_ID}`);
  params.set("cancel_url", `${origin}/call-center?checkout=cancel`);
  params.set("line_items[0][price]", priceId);
  params.set("line_items[0][quantity]", "1");
  params.set("client_reference_id", payload.sub);
  params.set("metadata[hd2d_checkout_kind]", "callcenter");
  params.set("metadata[hd2d_user_id]", payload.sub);
  params.set("subscription_data[metadata][user_id]", payload.sub);
  params.set("subscription_data[metadata][hd2d_checkout_kind]", "callcenter");
  if (payload.email) {
  ...
  if (!stripeRes.ok || !stripeJson.url) {
    ...
    console.error("[callcenter-checkout] stripe error:", stripeRes.status, msg);
    return new Response(JSON.stringify({ success: false, error: "Could not start checkout." }), {
      status: 502,
      headers: j,
    });
  }
