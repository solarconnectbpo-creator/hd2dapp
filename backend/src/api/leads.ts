...
export async function createLead(req: Request, env: Env, user: User) {
  try {
    ...
    if (!body.contactName || !body.phone) {
      return new Response(
        JSON.stringify({ error: "Missing contact name or phone" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        },
    ...
    // Geocode the address
    const geo = body.location
      ? await geocodeAddress(body.location, env)
      : { lat: null, lon: null };
...
export async function assignLead(req: Request, env: Env, user: User) {
  try {
    ...
    if (!leadId || !repId) {
      return new Response(
        JSON.stringify({ error: "Missing leadId or repId" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        },
    ...
    for (const row of leads) {
      try {
        ...
        const geo = row.location
          ? await geocodeAddress(row.location, env)
          : { lat: null, lon: null };
