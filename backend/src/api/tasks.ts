...
export async function createTask(req: Request, env: Env, user: User) {
  try {
    ...
    if (!dealId || !title) {
      return new Response(JSON.stringify({ error: "Missing dealId or title" }), {
        status: 400,
    ...
    ).bind(id, dealId, title, dueDate || null).run();
 ...
 */
export async function getTasks(req: Request, env: Env, user: User, dealId: string) {
  try {
 ...
 */
export async function completeTask(req: Request, env: Env, user: User, taskId: string) {
  try {
 ...
 * Delete a task
 */
export async function deleteTask(req: Request, env: Env, user: User, taskId: string) {
  try {
    await env.DB.prepare(
