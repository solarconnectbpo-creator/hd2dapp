...
export async function createDispute(req: Request, env: Env, user: User) {
  try {
    ...
    // Log audit
    await logAudit(env, user.id, "dispute_created", "vendor_dispute", {
      disputeId,
    });
...
export async function resolveDispute(req: Request, env: Env, user: User) {
  try {
    ...
    const lead = await env.DB.prepare("SELECT * FROM leads WHERE id = ?")
      .bind(dispute.lead_id)
      .first();

    // Get vendor details
    const vendor = await env.DB.prepare("SELECT * FROM companies WHERE id = ?")
      .bind(dispute.vendor_id)
      .first();
