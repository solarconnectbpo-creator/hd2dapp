  ...
  return (
    <div className="hd2d-page-shell max-w-4xl">
      <div className="mb-8">
        ...
        <h1 className="text-2xl font-semibold tracking-tight text-black">Buy leads</h1>
      ...
      {checkout === "success" ? (
        <div
          ...
          role="status"
        >
          <p className="font-medium text-black">Checkout completed.</p>
          ...
          {checkoutSessionId ? (
            <p className="mt-2 break-all font-mono text-xs text-black" title="Stripe Checkout session id">
      ...
      {!canPurchase ? (
        <p className="mb-6 text-sm text-[#8b9199]">
          Purchases are limited to <strong className="font-medium text-black">company</strong> and{" "}
          <strong className="font-medium text-black">admin</strong> accounts. Ask your org admin to upgrade your role or
      ...
      {packages.length === 0 ? (
        <Card className={cardChrome}>
          <CardHeader>
            <CardTitle className="text-black">No packages configured</CardTitle>
          ...
          <CardContent>
            <ol className="list-decimal space-y-3 pl-5 text-sm text-[#8b9199]">
              <li>
                <span className="text-black">Frontend (Vite):</span> set <code className={codeChip}>VITE_LEAD_PACKAGES_JSON</code>{" "}
              ...
              <li>
                <span className="text-black">Worker allowlist:</span> add every <code className={codeChip}>stripePriceId</code> to{" "}
                <code className={codeChip}>LEADS_STRIPE_PRICE_IDS</code> (comma-separated) on the Worker.
              </li>
              <li>
                <span className="text-black">Stripe:</span> set <code className={codeChip}>STRIPE_SECRET_KEY</code> as a Worker
                secret.
              </li>
              <li>
                <span className="text-black">Redirects:</span> set <code className={codeChip}>APP_PUBLIC_ORIGIN</code> on the
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {packages.map((pkg) => (
            <Card key={pkg.key} className={`flex h-full flex-col ${cardChrome}`}>
              <CardHeader>
                <CardTitle className="text-lg text-black">{pkg.title}</CardTitle>
                {pkg.priceLabel ? (
